...
def main(args):
    ...
    if args.testrail:
        ...
        if args.results:
            ...
            testrail_reconciler.get_testrun_results()
...
def persist_testrail_results():
    ...
    # if passing test result has no comments, add a default 'Passed testing in TestRail' comment to the Jira story before moving it along.

    # if passing test result DOES have comments, grab the text from them and put them in a Jira comment

    # if the passing test result has any attachments or screenshots, add those same attachments (assuming with a URL) to the Jira story

    # transition Jira stories to 'Ready for Staging Release' if all test cases in the release test run pass without incident.
